...
Stage 1: LLM(codex主・フォールバック連鎖)で全記事をスコアリング → TOP 30 選出
Stage 2: LLM で本文付き深い要約 → 記事8本生成
（LLM_PROVIDER で切替・llm.py の chat_json 経由）
"""
import json
...
[
  {
    "title": "見出し（日本語・40字以内）",
    "category": "カテゴリ（業界動向/ツール更新/新モデル発表/研究/セキュリティ/政策・規制 から最適な1つ。「その他」は禁止）",
    "source": "ソース名",
    ...
    "bizapp": {
      ...
      "actions": [
        "社内活用例：記事内容に最も合う実在ツールを絡めた実践アクション（30字以内）",
        "他社提案例：中小企業・採用・営業など現場に刺さる提案アイデア（30字以内）",
        "注目理由：放置すると何を失うか/いつまでに動くべきか（30字以内）"
      ]
    },
    "links": ["元記事URL"],
    "likes": 0
...
- actions[0] 社内活用：記事の内容に最も合う実在ツールを選ぶ。汎用チャット(Claude/ChatGPT)に逃げず、調査=NotebookLM/Felo、文書整備=Notion/Obsidian、コード=Cursor/Copilot、画像=Gemini/Canva のように記事固有のツールを優先。「△△の業務に使える」形式で
- actions[1] 他社提案：採用・営業・中小企業DX・バックオフィス自動化など現場目線の提案を
- actions[2] 注目理由：「〜が進む/変わる/課題」の抽象体言止めで終わらせない。"放置すると何を失うか・いつまでに動くべきか"を具体的に（例：来期予算策定前に調達計画を見直さないと値上がり直撃）
...
- 同じトピックの記事は1本にまとめる
- カテゴリは指定6種から選び「その他」を使わない（迷ったら最も近い1つ）
- 8本のcategoryが1種に偏らないよう、業界動向・ツール更新・研究などを織り交ぜる"""
